    ...
    <>
      <section className="border-b border-line bg-white">
        <Container className="py-16 lg:py-24">
          <Reveal>
            <p className="mb-3 text-xs font-semibold uppercase tracking-[0.2em] text-gold-dark">About {company.name}</p>
            <h1 className="max-w-3xl text-4xl font-extrabold leading-tight text-navy sm:text-5xl">
        ...
        <Container className="grid gap-12 lg:grid-cols-12">
          <Reveal className="lg:col-span-5">
            ...
            {company.markets.length > 0 && (
              <div className="mt-8">
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-gold-dark">Markets served</p>
                <ul className="mt-3 flex flex-wrap gap-2">
                  {company.markets.map((m) => (
                    <li key={m} className="rounded-full border border-line bg-white px-3 py-1 text-sm text-navy">{m}</li>
                  ))}
                </ul>
              </div>
            )}
      ...
      <section className="bg-navy py-20 text-cream lg:py-24">
        <Container>
          <Reveal>
            <SectionHeading light eyebrow="Product lines" title="Five categories, one point of contact." />
          </Reveal>
          <ul className="mt-12 grid gap-px overflow-hidden rounded-lg border border-cream/10 bg-cream/10 sm:grid-cols-2 lg:grid-cols-5">
            {categories.map((c, i) => (
              <Reveal key={c.slug} as="li" delay={i * 60} className="bg-navy p-6">
                <Link to={`/products?category=${c.slug}`} className="group block">
                  <CategoryIcon name={c.icon} className="h-6 w-6 text-gold" strokeWidth={1.4} />
                  <h3 className="mt-5 font-heading text-[15px] font-bold leading-snug">{c.name}</h3>
                  <span className="mt-3 inline-flex items-center gap-1 text-xs font-medium text-gold-light">
      ...
      <section className="py-20">
        <Container className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
          <div>
            <h2 className="text-3xl font-bold text-navy">Have a requirement in mind?</h2>
